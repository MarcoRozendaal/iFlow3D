...
class SedimentCapacityForcingMechanismCollection():
    """
    Class containing the sediment capacity forcing mechanisms of a certain order
    ...
    def __init__(self, forcing_mechanism_list: list):
        ...
        # Creates a nested dict first sorted on frequency component, then on forcing mechanisms and then on scaling
    ...
    def _generate_forcing_mechanism_nest_dict(self, forcing_mechanism_list):
        """ Generates a nested dict first sorted on frequency component, then on forcing mechanisms symbol and then on alpha:
            i.e. {1: {"A": {'a': forcing_mechanism, 'a_x': forcing_mechanism}}, 0: {"q": {'a': forcing_mechanism}, "gamma": {'a': forcing mechanism}}} """
        ...
        for forcing_mechanism in forcing_mechanism_list:
            ...
            alpha = forcing_mechanism.alpha

            # Using default dict behaviour
            forcing_mechanism_nest_ddict[n][symbol][alpha] = forcing_mechanism
